...
from mmdet.models.losses import ct_focal_loss, giou_loss, SmoothL1Loss
...
class CenterHead(AnchorHead):
    ...
    def __init__(self,
        ...
        self.off = self.build_head(2, off_head_conv_num, off_conv)
    ...
    def forward(self, feats):
        ...
        off = F.relu(self.off(x))

        return hm, wh, off

    @force_fp32(apply_to=('pred_heatmap', 'pred_wh', 'pred_off'))
    def get_bboxes(self,
                   pred_heatmap,
                   pred_wh,
                   img_metas,
    ...
    def loss(self,
        ...
        hm_loss, wh_loss, off_loss = self.loss_calc(pred_heatmap, pred_wh, pred_off, *all_targets)
        return {'losses/centernet_loss_heatmap': hm_loss, 'losses/centernet_loss_wh': wh_loss, 'losses/centernet_loss_off': off_loss}
    ...
    def target_single_image(self, gt_boxes, gt_labels, feat_shape):
        ...
        off_target_n = gt_boxes.new_zeros((self.wh_planes // 2, output_h, output_w))
        # off_target = gt_boxes.new_zeros((128, 2),dtype=torch.float32)
        # off_target_mask = gt_boxes.new_zeros(128, dtype=torch.int64)
        # ind = gt_boxes.new_zeros(128, dtype=torch.uint8)
        ...
        # ------------------------------------------
        off_value = ct - ct_ints.to(torch.float32)
        # ind = ct_ints[:, 1] * output_w + ct_ints[:, 0]
        # --------------------------------------------

        off_target_n[:, ct_ints[:, 0].long(), ct_ints[:, 1].long()] = off_value.permute(1, 0)
    ...
    def target_generator(self, gt_boxes, gt_labels, img_metas):
        ...
        with torch.no_grad():
            ...
            heatmap, box_target, reg_weight, off_target_n = multi_apply(
            ...
            heatmap, box_target, off_target = [torch.stack(t, dim=0).detach() for t in [heatmap, box_target, off_target_n]]
            reg_weight = torch.stack(reg_weight, dim=0).detach()

            return heatmap, box_target, reg_weight, off_target
    ...
    def loss_calc(self,
        ...
        off_loss = self.crit_off(pred_off, off_target,weight = mask_off, avg_factor=num_mask) * self.off_weight
